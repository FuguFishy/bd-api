...
def get_ops_dashboard(db: Session = Depends(get_db)):
    try:
        ...
        latest_workflow_rows = db.execute(
            text(
                """
                select
                    id,
        ...
        failed_workflow_rows = db.execute(
            text(
                """
                select
                    id,
        ...
        return {
            ...
            "latest_workflow_runs": [
                OpsDashboardWorkflowRun(**dict(row)) for row in latest_workflow_rows
            ],
            "failed_workflow_runs": [
                OpsDashboardWorkflowRun(**dict(row)) for row in failed_workflow_rows
            ],
            "open_review_items": [
